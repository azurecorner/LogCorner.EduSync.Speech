...
namespace LogCorner.EduSync.Speech.Domain.SpeechAggregate
{
    public class Speech : AggregateRoot<Guid>
    {
        ...
        public Speech(Title title, UrlValue urlValue, Description description, SpeechType type)
        {
            ...
            _mediaFileItems = new List<MediaFile>();
            AddDomainEvent(new SpeechCreatedEvent(Id, Title.Value, Url.Value, Description.Value, new SpeechTypeEnum(type.IntValue, type.StringValue), Guid.NewGuid(), DateTime.Now));
        }
        ...
        public void CreateMedia(MediaFile mediaFile, int originalVersion)
        {
            ...
            AddDomainEvent(new MediaFileCreatedEvent(Id, mediaFile.Id, mediaFile.File.Value, Guid.NewGuid(), DateTime.Now), originalVersion);
        }
        ...
        {
            AddDomainEvent(new SpeechDescriptionChangedEvent(Id, description.Value, Guid.NewGuid(), DateTime.Now), originalVersion);
        }

        public void ChangeUrl(UrlValue url, long originalVersion)
        {
            AddDomainEvent(new SpeechUrlChangedEvent(Id, url.Value, Guid.NewGuid(), DateTime.Now), originalVersion);
        }

        public void ChangeType(SpeechType type, long originalVersion)
        {
            AddDomainEvent(new SpeechTypeChangedEvent(Id, new SpeechTypeEnum(type.IntValue, type.StringValue), Guid.NewGuid(), DateTime.Now), originalVersion);
        }
        ...
        public void Delete(Guid id, long originalVersion)
        {
            ...
            AddDomainEvent(new SpeechDeletedEvent(Id, true, Guid.NewGuid(), DateTime.Now), originalVersion);
        }
